...
# -*- coding: utf-8 -*-
"""
Script para agregar el pedido de Test al sistema POS
"""
import sqlite3
import json
from datetime import datetime

def agregar_pedido_test():
    try:
        # Conectar a la base de datos del POS
        conn = sqlite3.connect('pos_pedidos.db')
        cursor = conn.cursor()
        
        # Datos del pedido de Test
        order_data = {
            'id': f'test_{int(datetime.now().timestamp())}',
            'cliente_nombre': 'Test',
            'cliente_telefono': '5464646642',
            'hora_recogida': '2:30 PM',
            'items': json.dumps([{"name": "Producto Mobile", "price": 65, "quantity": 1}]),
            'total': 65.0,
            'estado': 'pendiente',
        ...
        print(f"✅ Pedido de Test agregado exitosamente: {order_data['id']}")
